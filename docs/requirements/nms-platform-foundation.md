...
| Deployment (FR-54..58) | **PARTIAL 2026-08-09.** Stack LIVE on `10.121.77.206` — 11 rootless containers, no sudo, co-tenancy clean (disk 39→40%, off-limits listeners 4 throughout). **FR-58 = 4/5:** HTTPS reachable ✓, authenticated REST API ✓, simulated device discovered+polled (advancing counters, 99 RRD files) ✓, header-injection blocked ✓ — **but metrics-in-TSDB FAILED (F-2)** and **source-IP attribution FAILED (F-1)**. Three design findings routed to the Architect (F-1, F-2, F-5). | — | — |
