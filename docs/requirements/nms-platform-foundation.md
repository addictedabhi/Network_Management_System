...
| FR-16 | MUST — **CLOSED (satisfied-by-auto-provision), 2026-08-12** | Users SHALL be auto-provisioned in LibreNMS on first SSO login, with level derived from FR-15's mapping, and their level updated on subsequent logins when IdP claims change. **CLOSED as satisfied by LibreNMS's own `sso` mechanism, verified against real 25.7.0 source (design `nms-custdash-fr16.md` §2):** it auto-provisions the native user on first SSO login (`SSOAuthorizer::authenticate`, `create_users`) AND re-syncs the level from the group→level map on EVERY login (`SSOAuthorizer::getRoles` → `syncRoles` in `LegacyUserProvider::retrieveByCredentials`), covering both first-login provisioning and per-login level update. There is NO supported user-management REST API in `routes/api.php`, so a BFF-side provisioning call would have no endpoint and could only drift/conflict with `syncRoles` — building one is redundant, so none is built. The BFF `ensureUser` seam is kept as an honestly-documented no-op (`packages/bff/src/librenms/client.ts`). Re-opens as a NEW work item only if a future non-SSO IdP path is added. |
...
| G3 Assurance (Phase 2/2.5) | **Tester SIGN-OFF GRANTED 2026-08-11** at `ff76606` (deployed image `a7887192fa2f`). Full assurance pass over the Phase 2/2.5 diff `d315ace..ff76606`: code-review loop exited clean (2 iterations, 6 findings fixed); test loop found 1 HIGH (deploy-drift — running BFF stale vs reviewed source), resolved by rebuild from `git archive ff76606` with a pre-cutover dist gate, then re-verified clean (run-2). Executable cases all PASS (server-side authz negatives, FR-24 unavailable, three-states, pagination, CSP, bundle sweep, live devices). **4 authenticated-browser visuals BLOCKED (item 24)** — each with its server-layer control already proven; await a human credential-safe session OR human-driven G4 confirmation. Tester sign-off is necessary, not sufficient. | Human owns G4 | — |
