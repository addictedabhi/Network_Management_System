...
>
> **Revision 5 (2026-08-09) — F-2 / F-1 / F-5 finalization; TimescaleDB → InfluxDB v2 (OQ-3 re-answered).** This revision folds three deploy findings into the plan under the existing G2 (amend-and-proceed, no re-gate; the FR-58 re-run is the quality gate). It supersedes every TimescaleDB reference in Tasks 0.4c / 0.5 / 0.6. Authorities: **ADR 0009 (ACCEPTED — Option b, InfluxDB v2)**, **ADR 0005 rev 3-b**, **ADR 0003 (OQ-7 CLOSED)**, and `docs/design/nms-platform-foundation-deployment-findings.md` (§F-1, §F-2, §F-5). Human decisions recorded 2026-08-09.
>
> **F-2 — the TSDB is InfluxDB v2, NOT TimescaleDB. LibreNMS never wrote to TimescaleDB and never will (F-2: no PostgreSQL/TimescaleDB datastore driver in LibreNMS 25.7.0).** Every place below and in Tasks 0.4c/0.5/0.6 that says "TimescaleDB", "timescaledb", "Timescale", "TIMESCALE_PASSWORD", "hypertable", "5432", or "`psql`" is superseded as follows:
>   1. **Container (Task 0.4c Step 4 quadlet set):** `nms-timescaledb.container` (a pinned `timescale/timescaledb` image) is REPLACED by **`nms-influxdb.container`** on a **deliberately pinned `influxdb:2.x`** image — never `:latest`, never v1, never v3 (the 1.x/2.x/3.x split is a known hazard; pin v2 on both the container and the LibreNMS-config side). Same posture: **not published** to the host, reachable only on the `nms` network, credential/token server-side only (ADR 0002 / CON-6 hold in full — no browser->TSDB).
>   2. **LibreNMS datastore config:** configure LibreNMS's **`InfluxDBv2`** datastore (`lnms config:set datastores.influxdbv2.*` — URL, org, bucket, token) so LibreNMS writes natively over the line protocol. Delete the TimescaleDB write-path note in the manifest `README.md`; there is no bridge component (Option a was NOT chosen). **The plan no longer asserts any LibreNMS->TimescaleDB write path anywhere.**
>   3. **Retention as a PRECONDITION OF FIRST START (team-config §8 amended guardrail 4, unchanged in intent):** the 14-day retention is re-expressed as an **InfluxDB v2 bucket retention policy**, and it MUST be set **before the stack is first started** — the shared-disk / waived-snapshot precondition is identical to the former Timescale `drop_chunks` precondition, only the mechanism changes (bucket retention period, e.g. `14d`, set at bucket creation via `influx bucket create --retention 14d` or the container's `DOCKER_INFLUXDB_INIT_RETENTION`). An unbounded bucket on `vg_opt-lv_opt` (which also holds the third party's Kafka logs) is exactly the co-tenancy hazard the precondition exists to prevent.
>   4. **Remove the vestigial TimescaleDB container:** the deployed `nms-timescaledb` container carries **`count(*)=0`** — nothing ever wrote to it, so there is **NO data migration**. Remove the container AND its volume from `~/nms` with co-tenancy care (targeted `podman rm` / `podman volume rm` of the `nms-`-prefixed objects ONLY — **never** a blanket `podman system prune`, which would destroy the account's pre-existing `amx-mcp-server` images; team-config §8 guardrail).
>   5. **`.env` (Task 0.4c Step 4):** `TIMESCALE_PASSWORD` is replaced by the InfluxDB v2 init/admin credentials + operator token placeholders (`INFLUXDB_INIT_PASSWORD`, `INFLUXDB_INIT_ADMIN_TOKEN`, etc.) — placeholders only, never real values, `.env` git-ignored.
>   6. **SELinux label (Task 0.4c Step 4 rule 3):** InfluxDB v2 data is a **single-consumer** mount, so it keeps the **`:Z`** (private) label — the same class as the former TimescaleDB data mount. RRD stays `:z` (three consumers). Prefer a named Podman volume for the Influx data (sidesteps labelling/ownership), per the same rule.
>   7. **Task 0.4c Step 9 startup:** the `systemctl --user start …` line that named `nms-timescaledb` now names **`nms-influxdb`** instead.
>
> **F-2 — Task 0.6 Step 6 (the write-path verification) is re-expressed.** The old check (`podman exec nms-timescaledb psql … SELECT count(*) FROM <metrics_table>`) is REPLACED by an **InfluxDB v2 bucket read** proving metric points landed, e.g. `podman exec nms-influxdb influx query 'from(bucket:"<bucket>") |> range(start:-10m) |> count()'` — expected **> 0 points** for a polled simulated device. A zero count is a FAIL, not a warning (metrics would be RRD-only and every Phase-2 chart would query an empty store). RRD arrival (the first half of Step 6) is unchanged. **This is the F-2 half of the FR-58 re-run target (5/5).**
>
> **F-1 — source-IP attribution (findings §F-1).** Task 0.4c **Step 5's `containers.conf` `rootless_port_forwarder="pasta"` append is WRONG for named-bridge receivers and is proven ineffective** — delete/comment it (evidence marks it safe to delete). The real fix: move **ONLY** the two receiver sidecars `nms-snmptrapd` (1162/udp) and `nms-syslogng` (1514/udp) off the named `nms` bridge onto **pasta networking** (`Network=pasta` in their quadlet units, Task 0.4c Step 4); the other ~9 containers STAY on the `nms` bridge (aardvark-dns + nginx resolve-per-request depend on it). Receiver->MariaDB reachability per findings §F-1 (preferred: feed the LibreNMS pipeline; fallback: pasta pod + `-T`/`-U`). **Task 0.6 Step 7 asserts ATTRIBUTION** (event filed against the correct device), not mere receipt, with the source-IP probe re-run **per topology on the actual receiver units** — never inferred from `podman info` (that inference is the exact mistake that produced F-1). This is the F-1 half of the FR-58 re-run target. Full step list: findings §F-1.
>
> **F-5 — SSO cutover with break-glass (findings §F-5; ADR 0003 F-5 amendment).** Task 0.5 Steps 4–6 execute the controlled cutover **exactly per findings §F-5 step list**: (1) create/confirm a **local `mysql`-mechanism break-glass admin BEFORE touching `auth_mechanism`**; (2) register the dedicated **`nms` realm** + both OIDC clients (`nms-custom-ui`, `nms-native`) + the four groups; (3) verify datastore/LibreNMS isolation FIRST (Task 0.5 Step 3 — the Critical check; note there is **no 5432 listener** now, TimescaleDB is gone; the InfluxDB v2 container is likewise never published); (4) stand up the proxy as OIDC client, header-strip on, `/api/` exempt; (5) **dry-run `auth_test.php` while still on `mysql`**; (6) flip `auth_mechanism = sso` **LAST**; (7) post-cutover verification incl. unmapped->denied and logout-re-prompt; (8) one-step rollback (revert to `mysql` + reload). **OQ-7 CLOSED — the group->level map is `['nms-admin'=>10,'nms-engineer'=>10,'nms-operator'=>1,'nms-readonly'=>5]`, group-mapping fallback = 1, SSO no-match = `static_level 0` (no access).**
>
> **Sequencing for the maker (per the finalization handoff):** Influx swap + F-1 pasta sidecars FIRST and verified, **SSO cutover LAST** (a lockout must not block the Influx/pasta verification). Then RE-RUN FR-58 targeting the two current failures (metrics in InfluxDB v2; traps attributed by real source IP) -> goal 5/5, closing F-1 + F-2 and making FR-56 tickable.
>
> **What this revision changes:** supersedes all TimescaleDB references in Tasks 0.4c/0.5/0.6 -> InfluxDB v2; re-expresses Task 0.6 Step 6 (bucket read) and Step 7 (attribution); corrects Task 0.4c Step 5 (pasta topology split, not `containers.conf`); fixes the Task 0.4c Step 4 quadlet/`.env`/label references; folds in the F-5 cutover and OQ-7 values by reference to findings §F-5. **Tasks 0.2, 0.3 and 1–13 keep their numbers.** The disk-hazard table's "TimescaleDB / unbounded by default" row now reads "InfluxDB v2 / set a bucket retention period (POC: 14d) BEFORE first start". **No LibreNMS->TimescaleDB write path is asserted anywhere in the amended plan.**
...
- [ ] **Step 5: Add the TSDB — pinned** — **[REVISION 5: this is InfluxDB v2, NOT TimescaleDB — see the Revision 5 block at the top. This Task 0.2 Compose manifest is a 0.4a/0.4b-only artifact; the selected 0.4c branch authors its own units.]**
...
**[SUPERSEDED by Revision 5 — F-2: there is NO LibreNMS→TimescaleDB write path; LibreNMS 25.7.0 has no such driver. The store is InfluxDB v2, configured via the `InfluxDBv2` datastore (`lnms config:set datastores.influxdbv2.*`), and LibreNMS writes to it natively. Retention = a 14d bucket policy set BEFORE first start. No bridge component.]**
...
- [ ] **Step 6: Confirm metrics are landing — in RRD AND in InfluxDB v2** — **[REVISION 5 supersedes the TimescaleDB check below with an InfluxDB v2 bucket read; see the Revision 5 block at the top.]**
...
Then the check that F-2's resolution makes mandatory — **the LibreNMS→InfluxDB v2 write path is verified, not assumed** (ADR 0009 ACCEPTED / ADR 0005 rev 3-b). **[REVISION 5 — the store is InfluxDB v2; the `psql` count query below is SUPERSEDED by an InfluxDB v2 bucket read — e.g. `podman exec nms-influxdb influx query 'from(bucket:"<bucket>") |> range(start:-10m) |> count()'` expecting > 0 points. This is the F-2 half of the FR-58 re-run target (5/5).]**
